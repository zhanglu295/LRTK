...
def LRTK_usage(_Command1_, _Command2_ = "0"):
	...
	Basic options:
	-i --input, input file for fastq information (Three columns:1.Sample ID;2.Library ID;3. Path to sample fastqs).
...
###################################################### Config ##############################################################
	...
	if sys.argv[1] == "Config":
		...
		if os.path.isfile(nonN_region_bed):
			...
			subprocess.call(["python", FindNonBed_script, DatasetPahtDir + "/dataset/genome/genome.fa", nonN_region_bed])
...
####################################################### Varcall #############################################################
	...
	if runVAR > 0:
		...
		if Reseq_config != None and os.path.isfile(Reseq_config):
			...
			if os.path.isfile(Reseq_config) and os.path.isfile(Chrlist):
				pass
			else:
				sys.stderr.write("configuration file has not been provided or does not exist, Please create it using 'python LRTK-SEQ.py Config'\n" % Basic_config)
