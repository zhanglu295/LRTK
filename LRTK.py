...
def LRTK_usage(_Command1_, _Command2_ = "0"):
	...
	-m --molecule, number. molecule length less than it would be discarded [default: 0]
...
####################################################### MARK #################################################################
	...
	if runMAK > 0:
		...
		MarkBamFileList = Result_List_Dir + "/Basic_STAT_input.txt"
...
####################################################### STAT ################################################################
	...
	if sys.argv[1] == "Basicall":
		...
		runSTAT = 2
	if runSTAT > 0:
		opts, args = getopt.gnu_getopt(sys.argv[runSTAT:], 'i:o:m:p:c:s:d:', ['input', 'outputdir', 'molecule', 'parallel', 'config', 'softwarepath', 'datasetpath'])
		for o, a in opts:
			if runSTAT == 2:
				if o == '-i' or o == '--input':
					MarkBamFileList = a
		...
		for eachBamfile in rMarkBamFileList:
			...
			FilteredBamFile = bamfile.replace("merged.marked.bam", "merged.marked.filtered.bam")
