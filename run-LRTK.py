...
def LRTK_usage(_Command1_, _Command2_ = "0"):
	...
	Note: LRTK is allowed to modify configure.txt to include customerized parameters.
	...
	-i --input, the input path that contains compressed or uncompressed fastq files and sample info.
		file format:
		1. sample id
		2. library
		3. fastq path
	-o --outputdir, the output directory path
	-p --parallel, the number of CPU for parallelization (default: 1)
	...
	CFQ     generate clean fastq files and barcode error correction
 	ALN     reads alignment including bam sorting, indexing and barcode awared PCR duplication removal(must finishing CFQ)
 	MAK     merge bam files that belong to the same sample, mark PCR duplication based on barcode (must finishing ALN)
 	STAT    calculate QC statistics, including Cf, Cr, MuFL, NFP etc. (must finishing CFQ and ALN)
	...
	ALN_options = \
	'''
	-i --input, the input path that contains the clean fastq files generated by CFQ and sample info.
		file format:
		1. sample id
		2. library
		3. fastq 1 path
	...
	MAK_options = \
	'''
	-i --input, the input path that contains bam files from multiple lanes
		file format:
		1. sample id
		2. bam path
	-o --outputdir, the output directory path
	-p --parallele, the number of CPU for parallelization [default: 1]
	'''
	subhelpinfo["MAK"] = MAK_options
	...
	'''
	-i --input, the input path that contains the BAM files generated by QCall or ALN
	-v --vcf, unphased vcf file generated by Varcall, both compressed or uncompressed vcf files are allowed
	-o --outputdir, the output directory path
	'''
...
####################################################### CFQ #################################################################
	...
	if runCFQ > 0:
		...
		CFQ_script = ScriptDir + "/src/fastq_purification.py"
