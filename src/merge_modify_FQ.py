				...
				if id1[0] != ('@' + barcodeinfo[0]):
					...
					if barcode_err == 0:
						...
						if barcode_sequence != None:
							...
							fq2baminfo = "\t".join([readid, "0", "*", "0", "0", readlength1, "*", "0", "0", newfq_seq1, newfq_qua1, "RG:Z:" + barcode_sequence]) + "\n"
							wfq2bam.write(fq2baminfo)
							fq2baminfo = "\t".join([readid, "0", "*", "0", "0", readlength2, "*", "0", "0", newfq_seq2, newfq_qua2]) + "\n"
							wfq2bam.write(fq2baminfo)
					else:
...
if __name__ == '__main__':
	...
	process_num = 4
	barcode_seq = None
	opts, args = getopt.gnu_getopt(sys.argv[1:], 'i:2:o:c:b:z:n:h', ['input', 'input2', 'outputdir', 'config', 'barcode', 'splitsize', 'process_num', 'help'])
	for o, a in opts:
		if o == '-i' or o == '--input':
			inputfq = a
		if o == '-2' or o == '--input2':
